...
class ClipboardMonitor: SensitiveDetectorProtocol {
    ...
    private func checkClipboard() {
        ...
        // types.
        // The bundle-ID attribution above has a 0.5 s gap (the frontmost app
        // may have changed before the tick fires), so honor the type markers
        // unconditionally — capture nothing they stamp.
        let concealedTypes: [NSPasteboard.PasteboardType] = [
            NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType"),
            NSPasteboard.PasteboardType("org.nspasteboard.TransientType"),
            NSPasteboard.PasteboardType("org.nspasteboard.AutoGeneratedType")
        ]
        if let types = pasteboard.types, !Set(types).isDisjoint(with: concealedTypes) {
