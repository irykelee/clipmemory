...
### v2.2.4 (2026-07-16) — 릴리스 위생 수정

- **버전과 릴리스 태그 동기화** — `project.yml`의 `MARKETING_VERSION` 및 `CURRENT_PROJECT_VERSION`을 `2.2.4`로 업데이트하고 `project.pbxproj`를 재생성. v2.2.3에서 태그를 잘랐는데 버전은 업데이트하지 않아 다운스트림 cask가 이전 버전을 받는 문제를 수정
- **Quick Bar 라벨 수정** — Quick Bar「전체 창 열기」항목에서 오해의 소지가 있는 `⌘⌃V` 단축키 라벨을 제거. 글로벌 핫키는 전체 메인 창을 열고 Quick Bar는 메뉴바 📋 아이콘의 좌클릭으로 열림
- **문서의 핫키 설명 수정** — 8개 언어 README의 `Cmd+Ctrl+V` 설명을 다시 작성하여 Quick Bar가 아닌 메인 창을 여는 것임을 명확히 함
- **패키지 스크립트 안전성 강화** — `Scripts/package.sh`의 기본 버전 번호를 `project.yml`에서 `MARKETING_VERSION`을 읽도록 변경 (읽기 실패 가드 포함), 인수 없이 호출했을 때 이전 버전 번호의 tarball을 조용히 패키징하는 문제 방지
...
| 전체 창 열기 | `Cmd+Ctrl+V` (글로벌 핫키) / Quick Bar → "클립보드 열기" |
