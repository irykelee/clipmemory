...
### v2.9.2 (2026-09-24) — v2.9.2 — 시작 성능 최적화 및 감사 강화
...
- **🗂 백업 복원 마법사 (ID-BACKUP-0002)** — 설정 → 백업에 "백업에서 복원" 항목이 추가되었습니다. 5단계 마법사로 백업 선택(매일 자동 백업 목록 또는 외부 암호화 `.clipmemory` 패키지) → 비밀번호 입력(외부 패키지만) → 항목 수와 기간 미리보기 → 확인 → 결과 순으로 진행됩니다. 복원 시작 전에 안전 스냅샷을 자동으로 만들고, 비밀번호를 틀려도 그 자리에서 다시 시도할 수 있으며, 불완전한 백업은 미리 알려주고 디스크 공간이 부족하면 명확한 오류를 표시합니다. 이전에는 자동 백업에서 복구하려면 JSON 파일을 직접 옮겨야 했지만, 이제 전 과정에 UI가 있습니다.
- **🖼 이미지 공유 / 드래그 / 폴더로 내보내기 (ID-VIEW-0030 ~ 0038)** — 이미지 항목을 우클릭 → "공유…"하면 시스템 공유 시트(AirDrop / 메시지 / 메일 / 파일에 저장)가 열리고, 여러 개를 선택하면 "이미지 N개 공유…"로 바뀝니다. Finder나 다른 앱으로 바로 드래그할 수도 있습니다. 툴바 공유 메뉴에 "폴더로 내보내기…"가 추가되어 일괄 내보내기가 가능하며, 파일 이름이 겹치면 바꾸기 / 둘 다 유지 / 취소를 묻습니다 — 조용히 덮어쓰지 않습니다. 함께 메인 창 최소 너비를 850 → 950으로 올려, 창이 좁아도 툴바 버튼이 » 오버플로 메뉴로 접히지 않습니다.
- **📌 사이드바 "고정"에 개수 배지 표시 (ID-VIEW-0029)** — 다른 필터 탭과 표시를 맞춰 고정된 항목 수를 한눈에 확인할 수 있습니다.
- **⚖️ MIT LICENSE + 개인정보 처리방침 + 이용약관 추가 (ID-LEGAL-0001)** — `LICENSE`(MIT), `PRIVACY.md`, `TERMS.md`를 새로 추가했습니다. 그동안 공개 저장소였지만 라이선스가 없어 법적으로는 "모든 권리 유보"와 같았고, 누구도 안전하게 포크하거나 재배포할 수 없었습니다. 이제 라이선스 관계가 명확해졌고, 개인정보 처리방침에도 데이터가 기기에만 남는다는 점을 명시했습니다.
- **🌐 번역 일관성 회귀 방지 게이트 (ID-LINT-0001)** — `Scripts/lint-translations.sh`를 추가했습니다. `LocalizationService.swift`를 유일한 기준으로 삼아 221개 키 × 7개 언어를 모두 검증하며, 번역이 하나라도 빠지면 CI가 실패합니다(pre-commit과 CI 모두에 연결). 아울러 v2.7.x에서 남은 미사용 키 4개를 정리했습니다. 앞으로 새 기능이 번역 누락 상태로 배포되어 UI에 키 이름이 그대로 노출되는 일은 없습니다.

- v2.4.0 이후 자동 업데이트 모듈(Sparkle) 버전: 앱 내 자동 업데이트를 기다리거나 `brew upgrade --cask clipmemory`
- 전체 changelog: https://github.com/irykelee/clipmemory/releases/tag/v2.9.0
...
- **🍎 개인 정보 매니페스트 (PrivacyInfo.xcprivacy, ID-PRIVACY-0001)** — Apple 2024+에서 Mac App Store 또는 공증을 위한 모든 macOS 앱에 요구됨. 이번 릴리스에서 `NSPrivacyTracking=false`, 클립보드 + OCR 텍스트의 `NSPrivacyCollectedDataTypes` (linked, App Functionality 목적), 4가지 required-reason API를 선언하는 파일 추가. 사용자에게 투명함; Mac App Store를 통한 오픈 소스 배포의 규정 준수 장벽 제거.
- **🛠 4개의 잠재 버그 수정 (PR #61)** — 실제 수정: PR54-H chokepoint helper (itemIndex staleness window bounds check) + PR54-M1 locale pinning (Turkish/German diacritic cache poisoning) + PR56-M dead code 제거 (XCTest 격리 후 no-op) + PR56-L1 countLimit assert (미래 didSet 축소 방지).
- **📋 7 README 핫키 + L10n 일관성 (ID-DOCS-0001)** — 7개 언어 README 14개 위치의 `Cmd+Ctrl+V` 잔여 → `⌘⇧V` (`HotKeyManager.swift:10` defaultConfig `cmdKey | shiftKey` 일치) + 7 L10n `settings.hotkey.footer` "open main window" 통일.
- **🛡 Hotkey drift CI lint 활성화 (ID-CI-0002)** — `Scripts/lint-hotkey-drift.sh`가 7 README + 7 L10n footer의 `HotKeyManager.swift:10` 일관성을 자동 검증, 과거 drift 형식 금지. 향후 변경 → CI 빨강 → 6 레이어 동기화 강제.

- v2.4.0 이후 자동 업데이트 모듈(Sparkle) 버전: 앱 내 자동 업데이트를 기다리거나 `brew upgrade --cask clipmemory`
- 전체 changelog: https://github.com/irykelee/clipmemory/releases/tag/v2.8.4
...
| 전체 창 열기 | `⌘⇧V`(글로벌 단축키) / Quick Bar → "클립보드 열기" |
...
- 최대 기록 개수 (100/500/1000개)
- 민감 정보 자동 삭제 정책 (1시간/24시간/48시간/7일/안 함)
- 언어 전환 (7개 언어)
- 글로벌 핫키 녹음
- 외형 (라이트/다크/시스템 연동)
- 제외 앱 (클립보드 모니터링에서 제외할 앱 — 앱을 목록으로 끌어다 놓거나 선택기에서 고르며, 주요 암호 관리자는 기본 등록됨)
- Rich Text 캡처 토글
- 글꼴 크기 (소 / 중 / 대)
- 로그인 시 실행
