...
- **Textos da UI ainda exibiam inglês na inicialização para idiomas não ingleses** — O `didSet` do Swift não é acionado dentro de `init()`, e o novo espelho `currentLanguageCode` precisava ser semeado explicitamente para estar disponível desde o momento da inicialização.
- **`LanguageManager` agora usa espelho `nonisolated`** — Leitores off-main como `L10n.string()` (provenientes de `CryptoService.prepareKey` failure handler em `Task.detached`, etc.) leem o código do idioma sem cruzar a fronteira do main actor.
- Changelog completo: https://github.com/irykelee/clipmemory/releases/tag/v2.7.0
...
- **🗑️ Lixeira** — Itens apagados não são destruídos imediatamente. Eles vão para a Lixeira e ficam guardados por 7 dias (configurável em Ajustes), período durante o qual podem ser restaurados ou apagados permanentemente. Esvaziar a lixeira requer confirmação; itens expirados são limpos automaticamente.
