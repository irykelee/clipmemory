...
### v2.7.1 (2026-07-29) — 検索パフォーマンスと暗号化の安全性

- **検索速度が大幅に向上** — メインスレッドでの同期復号を廃止。キャッシュのウォームアップがアプリの復帰、新規アイテムの取得、リストの初回表示をカバーし、コールドスタート時の検索がもたつかなくなりました。
- **暗号化失敗の診断** — Keychain のロックにより復号に失敗した場合、検索ページに黄色の診断バナーが表示され原因を説明するようになり、静かに空の結果を返していた問題を改善しました。
- **復号失敗によるアイテムの永続的なマーク付けを修正** — 潜在的なバグを修正：Keychain が一時的にロックされた後、アイテムが「復号不可」と永続的にマークされていました。キーが復旧すると自動的に再試行されるようになりました。
- **DI コンテナの安全性** — `ServiceContainer.crypto` を `private(set)` に変更し、テストコード以外からの暗号化サービスの誤った置き換えを防止します。
- **AES-CBC 実装の統合** — ImageStorage 内の重複した CBC 復号実装を削除し、CryptoService の単一パスに統一しました。
- 完全な changelog: https://github.com/irykelee/clipmemory/releases/tag/v2.7.1
