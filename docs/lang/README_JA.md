...
### v2.2.4 (2026-07-16) — リリース衛生修正

- **バージョンとリリースタグの同期** — `project.yml` の `MARKETING_VERSION` と `CURRENT_PROJECT_VERSION` を `2.2.4` に更新し、`project.pbxproj` を再生成。v2.2.3 でタグを切ったのにバージョンを更新せず、下流 cask が旧バージョンを受け取る問題を修正
- **Quick Bar ラベル修正** — Quick Bar「フルウィンドウを開く」項目から誤解を招く `⌘⌃V` ショートカットラベルを削除。グローバルホットキーはフルメインウィンドウを開き、Quick Bar はメニューバー 📋 アイコンの左クリックで開く
- **ドキュメントのホットキー説明修正** — 8 言語の README にある `Cmd+Ctrl+V` の説明を書き直し、Quick Bar ではなくメインウィンドウを開くことを明確化
- **パッケージスクリプトの安全性強化** — `Scripts/package.sh` のデフォルトバージョン番号を `project.yml` から `MARKETING_VERSION` を読み取るように変更（読み取り失敗のガード付き）、引数なしで呼び出した際に古いバージョン番号の tarball を密かにパッケージ化する問題を防止
...
- 26ルールの自動機密検出（パスワード/APIキー/Slack/Discord/OpenAIトークン/身分証明書番号など）
...
- **スマート検出** — 26ルール（キーワード + 正規表現）でパスワード、APIキー、Slack/Discord/OpenAIトークン、秘密鍵、身分証明書番号などを自動識別
