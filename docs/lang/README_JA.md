...
### v2.9.2 (2026-09-24) — v2.9.2 — 起動パフォーマンス最適化と監査強化
...
- **🗂 バックアップ復元ウィザード (ID-BACKUP-0002)** — 設定 → バックアップ に「バックアップから復元」を追加。5 ステップのウィザードで、バックアップの選択（毎日の自動バックアップ一覧、または外部の暗号化 `.clipmemory` パッケージ）→ パスワード入力（外部パッケージのみ）→ 項目数と期間のプレビュー → 確認 → 結果、と進みます。復元開始前に安全スナップショットを自動取得し、パスワードを間違えてもその場で再試行でき、不完全なバックアップは事前に警告、ディスク容量不足は明示的なエラーになります。従来は JSON を手作業で戻すしかなかった経路に、UI が付きました。
- **🖼 画像の共有 / ドラッグ / フォルダへの書き出し (ID-VIEW-0030 ~ 0038)** — 画像項目を右クリック →「共有…」でシステムの共有シート（AirDrop / メッセージ / メール / ファイルに保存）を表示。複数選択時は「N 枚の画像を共有…」に変わります。Finder や他のアプリへ直接ドラッグすることもできます。ツールバーの共有メニューに「フォルダに書き出し…」を追加し、一括書き出しに対応。ファイル名が衝突した場合は 置き換える / 両方を残す / キャンセル を確認し、黙って上書きすることはありません。あわせてメインウィンドウの最小幅を 850 → 950 に引き上げ、狭い幅でもツールバーのボタンが » メニューに折り畳まれなくなりました。
- **📌 サイドバーの「ピン留め」に件数バッジ (ID-VIEW-0029)** — 他のフィルタタブと表示を揃え、ピン留め件数がひと目で分かるようになりました。
- **⚖️ MIT LICENSE + プライバシーポリシー + 利用規約を追加 (ID-LEGAL-0001)** — `LICENSE`（MIT）、`PRIVACY.md`、`TERMS.md` を新規追加。従来は公開リポジトリでありながらライセンス未設定で、法的には「全権利留保」と同義であり、安全にフォークや再配布ができませんでした。ライセンス条件が明確になり、プライバシーポリシーにもデータが端末内に留まることを明記しています。
- **🌐 翻訳整合性のリグレッションゲート (ID-LINT-0001)** — `Scripts/lint-translations.sh` を追加。`LocalizationService.swift` を唯一の正とし、221 個のキー × 7 言語をすべて検証します。翻訳が 1 つでも欠けると CI が赤くなります（pre-commit と CI の両方に組み込み済み）。あわせて v2.7.x 由来の未使用キー 4 個を削除しました。今後、未翻訳のロケールが UI にキー名を露出させることはありません。

- v2.4.0 以降の自動更新モジュール（Sparkle）搭載バージョン：アプリ内自動更新を待つ、または `brew upgrade --cask clipmemory`
- 完全な changelog: https://github.com/irykelee/clipmemory/releases/tag/v2.9.0
...
- **🌏 中国ユーザーは Gitee ミラーソースに切替可能** — 設定 → アップデートとこのアプリについて → アップデートソース → ミラー (Gitee)；Gitee ミラーは appcast とインストールパッケージの両方をホスティング、国内ネットワークから VPN なしで更新チェック可能
...
設定 → アップデートとこのアプリについて → アップデートソース → **ミラー (Gitee)** — GitHub に到達できない環境でも、国内ネットワークから 更新チェックとダウンロードが可能。Gitee ミラーは appcast とインストール パッケージの両方をホスティング（jsDelivr は appcast のみ）するため、ダウンロードも国内完結。EdDSA 署名検証は GitHub ソースと同じ。
...
| フルウィンドウを開く | `⌘⇧V`（グローバルホットキー）/ Quick Bar → "クリップボードを開く" |
...
- 最大履歴件数（100 / 500 / 1000件）
- 機密情報自動クリア（1時間/24時間/48時間/7日/なし）
- 言語切替（7言語）
- グローバルホットキー録音
- 外観（ライト/ダーク/システム連動）
- 除外アプリ（クリップボード監視から除外するアプリ —— アプリをリストにドラッグするか、ピッカーから選択。主要なパスワード管理アプリは初期登録済み）
- Rich Text キャプチャ トグル
- フォントサイズ（小 / 中 / 大）
- ログイン時に起動
