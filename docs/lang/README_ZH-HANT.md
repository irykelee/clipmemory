...
- **搜尋不再卡頓** — 搜尋含富文字的歷史時，先前會在主執行緒逐條解密導致明顯卡頓；現在冷快取條目先跳過、背景預熱完成後自動出現在結果中
- **QuickBar 搜尋更完整** — 搜尋時未解密的條目先前會被靜默漏掉且不會補全；現在預熱完成後結果自動重新整理補全
- **重複條目自動合併** — 啟動金鑰就緒後，歷史中的重複條目（含先前啟動視窗期漏網累積的）現在會自動合併清理
- **圖片瀏覽更快** — 圖片讀取不再被背景的舊格式遷移任務阻塞
- **修復資源回收筒條目整段工作階段空白** — 登入自啟動且金鑰鏈尚未解鎖時，資源回收筒的文字/連結條目先前一直空白且不會自行修復
- 完整 changelog: https://github.com/irykelee/clipmemory/releases/tag/v2.7.7

### v2.7.6 (2026-08-01) — 穩定性與資料安全加固

- **過期自動清理改為進資源回收筒，置頂條目永久豁免** — 達到保留上限的自動清理此前會永久刪除條目；現在改為移入資源回收筒（可隨時找回），且置頂（收藏）條目不再被自動清理
...
- **修復資源回收筒操作崩潰** — 刪除或還原資源回收筒內項目時可能觸發崩潰（或靜默操作到錯誤的項目），現已修復
- **修復防抖儲存定時器靜默失效** — 標籤編輯、資源回收筒操作等非即時落盤路徑的防抖儲存，在首次觸發後會停止運作；崩潰或強制結束將遺失上次啟動後的全部標籤/資源回收筒變更
- **修復含資源回收筒條目的備份無法匯入** — 任何包含非空資源回收筒的備份檔在匯入時都會失敗，現已相容
