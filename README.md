...
- **🗂 备份还原向导 (ID-BACKUP-0002)** — 设置 → 备份 新增「从备份还原」入口，5 步可视化向导：选备份（每日自动备份列表，或外部 `.clipmemory` 加密包）→ 输密码（仅外部包需要）→ 预览条目数与时间范围 → 确认 → 结果。还原开始前自动做一次安全快照；密码输错原地重试；备份不完整提前提示，磁盘空间不足给明确报错。此前从自动备份恢复只能手工搬 JSON，现在全程有 UI。
- **🖼 图片分享 / 拖拽 / 导出到文件夹 (ID-VIEW-0030 ~ 0038)** — 图片条目右键「分享…」调起系统分享面板（隔空投送 / 信息 / 邮件 / 存储到文件），多选时变「分享 N 张图片…」；也可直接把图片拖到访达或其他 App；工具栏分享菜单新增「导出到文件夹…」批量导出，重名弹 替换 / 两者保留 / 取消，绝不静默覆盖。配套主窗口最小宽度 850 → 950，窄窗口下工具栏按钮不再折进 » 溢出菜单。
- **📌 侧边栏「置顶」显示条数徽章 (ID-VIEW-0029)** — 与其他筛选 tab 对齐，一眼看到置顶了几条。
- **⚖️ 补齐 MIT LICENSE + 隐私政策 + 使用条款 (ID-LEGAL-0001)** — 新增 `LICENSE`（MIT）、`PRIVACY.md`、`TERMS.md`。此前项目公开但无许可证，法律上等同「保留所有权利」，别人无法安全 fork 或二次分发；现在授权关系明确，隐私政策也白纸黑字写明数据只留本机。
- **🌐 翻译一致性防回归门 (ID-LINT-0001)** — 新增 `Scripts/lint-translations.sh`，以 `LocalizationService.swift` 为唯一真相源校验 221 个 key × 7 语种，缺一个翻译 CI 直接红（已接进 pre-commit 与 CI），并清掉 4 个 v2.7.x 遗留死 key。以后新功能不会再出现某语种漏翻、界面蹦出英文 key 名。
...
- **🆕 回收站批量恢复（NEW-batch-restore）** — Trash tab 现在支持多选 + 一键恢复：行内 checkbox + 顶部 master checkbox（全选/全不选/部分选中三态）+ Shift+点击选区 + Restore 按钮根据选中数动态显示「Restore N items」。「一条一条点」历史结束。
...
- **🛠 import 溢出走回收站（M-2）** — `importBackupItems` 检测到导入后条目数 > maxItems 时，溢出条目经 `moveToTrash` 走回收站（保留可恢复）而非直接丢弃。
- **🛠 7 项 audit 驱动的"用系统默认"重构（PR #40-#47）** — SelectCheckbox/CloseButton 共享组件抽取 + NSWindow.setFrameAutosaveName 替换手写 JSON + Notification.Name 注册表补漏 + 4 处 keyCode → Carbon `kVK_*` + 搜索防抖统一 250ms + sz() clamp 注释 + L24 sweep。
...
| 复制条目 | 点击条目 / 键盘 ↑↓ + Enter |
| 打开完整窗口 | `⌘⇧V`（全局快捷键）/ Quick Bar → "打开完整窗口" |
| 搜索 | 输入关键词，匹配处高亮 |
| 收藏 / 取消收藏 | 点击 ⭐ 或双击条目 |
| 删除 | 点击 🗑 或右键菜单 |
...
- 历史记录最大条数（100 / 500 / 1000 条）
- 敏感信息清除策略（1 小时 / 24 小时 / 48 小时 / 7 天 / 不自动清除）
- 语言切换（7 种语言）
- 全局快捷键录制
- 外观（浅色 / 深色 / 跟随系统）
- 排除应用（不监控的 App —— 把 App 拖进列表，或从选择器挑；已预置常见密码管理器）
- 富文本捕获开关
- 字体缩放（小 / 中 / 大）
- 开机自启
- 回收站保留期（3 / 7 / 14 / 30 天）
