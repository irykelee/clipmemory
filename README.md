...
| **主界面** | 固定宽度的 ContentView，无侧边栏 | **固定侧边栏**：全部 / 文本 / 图片 / 链接 / 已收藏 / 偏好设置 |
...
| **红绿灯** | 标题栏内 | 标题栏隐藏，统一工具栏区域显示（macOS 26 Liquid Glass 风格） |
| **Dock 图标** | 一直隐藏 | 打开窗口时出现 Dock 图标，关闭后消失 |
| **右键高亮** | 不高亮 | 悬停自动高亮 |
| **字体缩放** | 无 | 设置页支持小/中/大三档全局字体缩放 |
| **开机自启** | 无（仅菜单栏操作） | 设置页新增「登录时启动」 |
...
### 主题系统
设置页可调整窗口效果（实心 / 毛玻璃 / 高透）和外观（浅色 / 深色 / 跟随系统）。
