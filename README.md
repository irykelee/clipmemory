...
- **修复垃圾桶操作崩溃** — 删除或恢复垃圾桶内项目时可能触发崩溃（或静默操作到错误的项目），现已修复
- **修复防抖保存定时器静默失效** — 标签编辑、垃圾桶操作等非即时落盘路径的防抖保存，在首次触发后会停止工作；崩溃或强制退出将丢失上一次启动后的全部标签/垃圾桶变更
- **修复含垃圾桶条目的备份无法导入** — 任何包含非空垃圾桶的备份文件在导入时都会失败，现已兼容
